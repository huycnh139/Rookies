...
function Product() {
  ...
  const productImage = (id) => {
    let _image = images.filter(
      (val) => val.productId === id && val.isDefualt == true
    )[0];
    if (_image === null) {
      _image = {
        id: id,
  ...
  const statusBodyTemplate = (rowData) => {
    return (
      <span
        className={`product-badge status-${flagStock(
          rowData.stock
        ).toLowerCase()}`}
      >
        {flagStock(rowData.stock)}
      </span>
    );
  };
  const createdBodyTemplate = (product) => {
    return new Date(product.dateCreate).toLocaleDateString();
  };
  const updateBodyTemplate = (product) => {
    return new Date(product.dateUpdate).toLocaleDateString();
  };
        ...
        <div className="field">
          ...
          <div className="formgrid grid">
            <InputNumber
              ...
              onValueChange={(e) => onInputNumberChange(e, "categoryId")}
              integeronly="true"
            />
          </div>
        </div>
